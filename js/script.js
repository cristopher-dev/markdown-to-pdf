...
function updateHighlightTheme(selectedThemeOption, currentBsTheme) {
  ...
  if (currentBsTheme === 'cristopher') {
    // For Cristopher's theme, we use a specific highlight.js theme that combines well
    cssFile = selectedThemeOption.dataset.cristopher || selectedThemeOption.dataset.dark || 'ocean.min.css';
  } else {
    cssFile = currentBsTheme === 'dark'
...
function applyThemeSetting(theme) {
  ...
  // Update highlight.js theme if one has already been selected
  ...
  if (selectedHighlightTheme && selectedHighlightTheme.value !== '') {
    updateHighlightTheme(selectedHighlightTheme, effectiveTheme);
  }
}
...
// Change highlight.js theme when a new one is selected
